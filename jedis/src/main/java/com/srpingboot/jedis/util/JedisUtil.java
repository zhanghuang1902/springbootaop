package com.srpingboot.jedis.util;
...
public class JedisUtil {
	...
	private static JedisPool jedisPoolStatic;

	@Autowired
	public void init(JedisPool jedisPool){
		JedisUtil.jedisPoolStatic = jedisPool;
	}

	/**
	 * 模糊查询key列表
	 * @param key 查询条件，拼上*就是模糊查询
	 * @param indexdb 数据库index
	 * @return key列表
	 */
	public static Set<String> getKeySet(String key, int indexdb) {
		Jedis jedis = null;
		try {
	 ...
	 * @param key 查询条件，拼上*就是模糊查询
	 * @param indexdb 数据库index
	 * @return key列表 如果没有则返回null
	 */
	public static List<String> getValues(String key, int indexdb) {
		try {
			Set<String> keySet = getKeySet(key,indexdb);
			if(keySet == null || key.isEmpty()){
				return null;
			}
			String[] keys = keySet.toArray(new String[keySet.size()]);
			List<String> values = mget(keys,indexdb);
			return values;
		}catch (Exception e){
			throw e;
		}
	 ...
	 */
	public static String set(String key, String value, int expire, int indexdb) {
		String returnValue = set(key, value, indexdb);
		expire(key, expire, indexdb);
		return returnValue;
	}
	 ...
	 * @return
	 */
	public static String getLoginMobileCode(String clientId,String mobile){
		return get("loginCode_"+clientId+"_"+mobile,0);
	}
	 ...
	 */
	public static List<String> mget(String[] keys,int indexdb) {
		Jedis jedis = null;
	 ...
	 * @return 加值后的结果
	 */
	public static Long incr(String key,int indexdb,int expire) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = jedisPoolStatic.getResource();
...
//		if (jedis != null) {
//			jedisPoolStatic.returnResource(jedis);
//		}
//	}


	public static Jedis getJedis(){
		Jedis jedis = jedisPoolStatic.getResource();
		jedis.select(0);
		return jedis;
	}
